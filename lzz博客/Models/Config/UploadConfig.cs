...
namespace lzz博客.Models.Config
{
    ...
    public class UploadConfig:ConfigurationSection
    {
        ...
        private static ConfigurationProperty _property = new ConfigurationProperty(string.Empty,
            typeof(KeyValueElementCollection), null, ConfigurationPropertyOptions.IsDefaultCollection);
        /// <summary>
        /// 配置列表
        /// </summary>
        ...
        private KeyValueElementCollection KeyValues
        {
            get
            {
                return (KeyValueElementCollection)base[_property];
            }
            set
            {
        ...
        public int MaxSize
        {
            get
            {
                ...
                if (KeyValues["MaxSize"] != null)
                    int.TryParse(KeyValues["MaxSize"].Value, out _value);
                return _value;
            }
            set
            {
                if (KeyValues["MaxSize"] == null)
                {
                    KeyValues["MaxSize"] = new KeyValueElement()
                    {
                        Key = "MaxSize",
        ...
        public string Path
        {
            ...
            {
                if (KeyValues["Path"] == null)
                    KeyValues["Path"] = new KeyValueElement()
                    {
                        Key = "Path",
                        Value = value
                    };
